...
ALTER PROC [dbo].[MassiveCharge] AS
BEGIN

/*+ Creación de la tabla Temporal que contendrá los Repuestos con sus precios*/
	IF OBJECT_ID('tempdb..#TMP_RESPUESTO') IS NOT NULL DROP TABLE #TMP_RESPUESTO
    CREATE TABLE #TMP_RESPUESTO (Nombre VARCHAR(100),Precio DECIMAL(18,6))
	IF OBJECT_ID('tempdb..#TMP_RESPUESTOS_NO_INGRESADOS') IS NOT NULL DROP TABLE #TMP_RESPUESTOS_NO_INGRESADOS
	CREATE TABLE #TMP_RESPUESTOS_NO_INGRESADOS (Nombre VARCHAR(100),Precio DECIMAL(18,6))
...
DECLARE @nombre varchar(100), @precio decimal -- Here we create a variable that will contain the ID of each row.
...
FETCH NEXT FROM ITEM_CURSOR INTO @nombre, @precio -- We fetch the first result
...
BEGIN
 ...
 IF(@precio<20)
 BEGIN
	INSERT INTO TallerMecanico.dbo.repuestos values(@nombre, @precio)
 END 
 ELSE
 BEGIN
	INSERT INTO #TMP_RESPUESTOS_NO_INGRESADOS values(@nombre, @precio)
 END
 
FETCH NEXT FROM ITEM_CURSOR INTO @nombre, @precio -- Once the work is done we fetch the next result
...
SELECT * FROM #TMP_RESPUESTOS_NO_INGRESADOS
END
GO
